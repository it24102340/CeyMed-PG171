...
public interface PurchaseRepository extends JpaRepository<Purchase, Long> {
    ...
    @Query("SELECT p FROM Purchase p WHERE p.status = com.example.insurance.entity.PurchaseStatus.ACTIVE")
    List<Purchase> findActivePurchases();

    @Query("SELECT p FROM Purchase p WHERE p.user.id = :userId AND p.status = com.example.insurance.entity.PurchaseStatus.ACTIVE")
